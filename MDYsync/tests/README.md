...
| `tests/notes/select-text-highlight.spec.mjs` | The rendered `.vilna-select-text-selection-rect` -- reported directly as "too narrow" and "clipped, uneven" next to the blue "now playing" highlight, even though both draw from the identical `groupBoxesIntoLineRects` geometry. Locks the selection rect to the same solid-fill/multiply-blend/999px-pill treatment `.vilna-active-rect` already has (computed styles, not a screenshot -- this fixture never loads a real page image), and that only the color still differs |
| `tests/notes/select-word-phrase.spec.mjs` | "Select this word"/"Select whole phrase" -- two context-menu items that start a Select-text selection directly from a right-click/long-press. Covers `selectVilnaWord`/`selectVilnaPhrase` (app.js): replacing vs. extending an existing selection, the whole-ref/single-word fallbacks when a segment has no word-level bounds or none at all, and that the resulting selection can still be dragged past its own starting bounds; that the "now playing" blue highlight defers to (excludes) whatever the selection currently covers, reported directly as needing to replace the phrase's blue sync highlight with yellow while selected; and the menu wiring itself (`daf-context-menu.js`) -- both items only on the Vilna page, and always act on the literal word under the pointer (`target.word`) even when a right-click landed inside an already-selected passage |
| `tests/notes/shas-search.spec.mjs` | "Search in Shas" (shas-search.js), on top of Sefaria's own real `POST /api/search-wrapper` (confirmed directly against the live endpoint while building this): query cleaning (nikud/cantillation stripped, Hebrew punctuation like a maqaf becomes a space rather than gluing its neighbors together -- a subtlety caught the hard way, since both live in the same Unicode block), safely turning Sefaria's own `<b>` snippet markup into real `<mark>` elements without ever executing anything else in an external response, deduping repeat versions of the same ref, the dialog's loading/empty/error states and an honest truncation note, the context menu item, and opening a result end to end -- navigates to `/browse/?ref=...&hlRef=...`, and the destination flashes and auto-scrolls to the exact matched segment via `loadDaf`'s own Sefaria-fallback path (not gated on a synced video, unlike the picker's own `onDafPickerChanged`), with a "Back to results" banner |
...
| `tests/player/vilna-tap-highlight.spec.mjs` | Tapping a word on the Vilna page (or a scanned photo) must move the "now playing" highlight to that word, not just seek the video there -- reported directly as the highlight staying stuck on the previous phrase. Simulates a real YouTube player's asynchronous `seekTo()` (an HTML5 `<video>`'s synchronous-enough `currentTime` setter is why this never showed up there, and every daf in this project with a real recording is a YouTube shiur) |
...
| `tests/player/vilna-gemara-block.spec.mjs` | `restrictWordBoxesToGemaraBlock` (app.js) drops any word box whose OCR-matched position falls outside the page's own Gemara text block -- reported directly as the highlight sometimes landing on Rashi/Tosafot, which routinely quote the Gemara verbatim and so pass the OCR pipeline's purely-textual match against the *wrong* physical occurrence. Covers the filter itself (inside/outside/straddling the block edge, and graceful no-op when a pre-v2 results file has no `textBlock` at all) and that it's wired into both `loadVilnaPageMap`'s fetch pipeline and what `renderVilnaWordBoxes` draws |
| `tests/player/vilna-toolbar-overlap.spec.mjs` | The Vilna page toolbar's Select Text button and Fullscreen button -- reported directly as overlapping on `/player/` and `/browse/`. Both pages give `#vilnaZoomControls` a compact pill layout via a page-scoped `<style>` block that squeezes every `.vilna-zoom-btn`-classed control to a 30x30 icon square, which caught `#vilnaSelectTextModeButton` too even though it holds a text label; with no `overflow: hidden`, the label painted past its own layout box into the neighboring button. Covers both pages plus `/watch/` (no such override, the control case) for true bounding-box overlap, and that the label isn't squeezed to icon width in the first place -- the assertion that actually catches the regression, since `boundingBox()` measures the layout box, not painted text overflow |
| `tests/player/vilna-ink-bands.spec.mjs` | `measureInkBands` (app.js) -- reported directly as some highlighted lines still rendering very thin even after the selection rect was fixed to match the blue one. Draws a synthetic printed page straight onto a canvas (no PDF, no real OCR needed) with one line noticeably lighter than its neighbours and checks it isn't clipped down from its real height by the single global darkness threshold -- and a regression guard for the exact bug that surfaced while fixing it: an even count of raw bands averages into a fractional row count, which a typed array silently reads back as `undefined` rather than throwing |
